import React, { createContext, useContext, useState, useEffect } from "react";
import { base44 } from "@/api/base44Client";

const LanguageContext = createContext();

export const useLanguage = () => {
  const context = useContext(LanguageContext);
  if (!context) {
    throw new Error("useLanguage must be used within LanguageProvider");
  }
  return context;
};

// Translations object
const translations = {
  en: {
    // Navigation
    home: "Home",
    myIdea: "My Idea",
    findInvestors: "Find Investors",
    learningHub: "Learning Hub",
    cofounderConnect: "Co-founder Connect",
    community: "Community",
    meetingNotes: "Meeting Notes",
    appointments: "Appointments",
    profile: "Profile",
    findPitches: "Find Pitches",
    investorConnect: "Investor Connect",
    crowdfunding: "Crowdfunding",
    crowdfundingCampaigns: "Crowdfunding Campaigns",
    exploreActiveCampaigns: "Explore active campaigns and see who's investing",
    activeCampaigns: "Active Campaigns",
    totalRaised: "Total Raised",
    totalBackers: "Total Backers",
    funded: "Funded",
    noActiveCampaigns: "No active campaigns yet",
    checkBackSoon: "Check back soon for new crowdfunding opportunities!",
    whatsapp: "WhatsApp",
    email: "Email",
    disconnect: "Disconnect",
    connected: "Connected",
    notProvided: "Not provided",
    investmentFocus: "Investment Focus",
    portfolioDisclaimer: "This is a simulated projection to demonstrate potential growth after investor connections.",
    
    // Profile page
    myProfile: "My Profile",
    yourPersonalDetails: "Your personal details",
    fullName: "Full Name",
    email: "Email",
    phoneNumber: "Phone Number",
    location: "Location",
    businessName: "Business Name",
    businessType: "Business Type",
    preferredLanguage: "Preferred Language",
    aboutMe: "About Me",
    saveProfile: "Save Profile",
    logout: "Logout",
    selectLanguage: "SELECT LANGUAGE",
    notSet: "Not set",
    yourPhoneNumber: "Your phone number",
    villageCityState: "Village / City, State",
    yourBusinessName: "Your business name",
    selectBusinessType: "Select your business type",
    tellUsAboutYourself: "Tell us about yourself and your business...",
    
    // Business types
    handicrafts: "🎨 Handicrafts",
    textiles: "🧵 Textiles",
    food: "🍳 Food",
    agriculture: "🌾 Agriculture",
    retail: "🛍️ Retail",
    services: "💼 Services",
    other: "📦 Other",
    
    // Support
    tollFreeSupport: "Toll-Free Support",
    available24x7: "Available 24/7",
    
    // Investor Dashboard
    welcomeBack: "Welcome back",
    discoverInvest: "Discover and invest in promising women entrepreneurs",
    activePitches: "Active Pitches",
    entrepreneurs: "Entrepreneurs",
    opportunities: "Opportunities",
    growthRate: "Growth Rate",
    explorePitches: "Explore Pitches",
    browseInnovative: "Browse through innovative business ideas from women entrepreneurs",
    viewAllPitches: "View All Pitches",
    myPortfolio: "My Portfolio",
    trackInvestments: "Track your investments and connected entrepreneurs",
    viewPortfolio: "View Portfolio",
    recentPitches: "Recent Pitches",
    viewAll: "View All",
    fundingNeeded: "Funding needed",
    noPitchesAvailable: "No recent pitches available",
    discoverInnovative: "Discover innovative business ideas from women entrepreneurs",
    searchPitches: "Search pitches...",
    allCategories: "All Categories",
    loadingPitches: "Loading pitches...",
    noPitchesFound: "No pitches found",
    viewDetails: "View Details",
    coFounderConnector: "Co-Founder Connector",
    findPerfectPartner: "Find the perfect business partner to grow together",
    aiPoweredMatching: "AI-Powered Matching",
    aiAnalyzes: "Our AI analyzes your skills, experience, and goals to suggest the best co-founder matches",
    searchByName: "Search by name, skills, or location...",
    activeCoFounders: "Active Co-Founders",
    skillsAvailable: "Skills Available",
    successfulMatches: "Successful Matches",
    otherConnect: "Other Connect",
    fullTime: "Full-Time",
    partTime: "Part-Time",
    skills: "Skills",
    lookingFor: "Looking For",
    yearsExperience: "years experience",
    connect: "Connect",
    pitch: "Pitch",
    loadingCoFounders: "Loading co-founders...",
    noCoFoundersFound: "No co-founders found",
    trackYourInvestments: "Track your investments and connections",
    overview: "Overview",
    profileDetails: "Profile Details",
    connections: "Connections",
    activity: "Activity",
    totalInvestments: "Total Investments",
    connectedEntrepreneurs: "Connected Entrepreneurs",
    rating: "Rating",
    status: "Status",
    active: "Active",
    investorProfile: "Investor Profile",
    focusAreas: "Focus Areas",
    investmentRange: "Investment Range",
    investorType: "Investor Type",
    profilePicture: "Profile Picture",
    completeInvestorDetails: "Complete Investor Details",
    fullName: "Full Name",
    bio: "Bio",
    yourFullName: "Your full name",
    tellAboutYourself: "Tell us about yourself and your investment philosophy...",
    company: "Company/Organization",
    yourCompany: "Your company or investment firm",
    linkedinUrl: "LinkedIn URL",
    website: "Website",
    saveProfile: "Save Profile",
    noConnectionsYet: "No connections yet",
    entrepreneur: "Entrepreneur",
    contact: "Contact",
    recentActivity: "Recent Activity",
    noRecentActivity: "No recent activity",
    dashboard: "Dashboard",
    
    // Home page
    empoweringWomen: "Empowering Women",
    getStarted: "Get Started",
    womenWhoMadeIt: "Women Who Made It Happen",
    voiceFirst: "Voice-First Interface",
    aiCoaching: "AI Coaching",
    investorConnections: "Investor Connections",
    
    // Community
    celebratingAmazingWomen: "Celebrating our amazing women entrepreneurs",
    topInnovator: "Top Innovator",
    bestPitch: "Best Pitch",
    communityHelper: "Community Helper",
    cofounderConnector: "Co-Founder Connector",
    recognizingExcellence: "Recognizing excellence",
    topEntrepreneursThisMonth: "Top Entrepreneurs This Month",
    
    // RoleSelect page
    shakti: "Shakti",
    empoweringRuralWomen: "Empowering rural women to turn their dreams into thriving businesses",
    selectYourRole: "Select Your Role",
    imAnEntrepreneur: "I'm an Entrepreneur",
    entrepreneurDescription: "I have a business idea and want to share it with the world, find investors, and grow",
    imAnInvestor: "I'm an Investor",
    investorDescription: "I want to discover promising business ideas and support rural women entrepreneurs",
    watchDemo: "Watch Demo",
    needHelp: "Need Help?",
    callTollFree: "Call Toll-Free",
    selectLanguageToStart: "Select your language to get started",
  },
  hi: {
    // Navigation
    home: "होम",
    myIdea: "मेरा विचार",
    findInvestors: "निवेशक खोजें",
    learningHub: "शिक्षा केंद्र",
    cofounderConnect: "सह-संस्थापक कनेक्ट",
    community: "समुदाय",
    meetingNotes: "बैठक नोट्स",
    appointments: "नियुक्तियाँ",
    profile: "प्रोफ़ाइल",
    findPitches: "पिच खोजें",
    investorConnect: "निवेशक कनेक्ट",
    crowdfunding: "क्राउडफंडिंग",
    crowdfundingCampaigns: "क्राउडफंडिंग अभियान",
    exploreActiveCampaigns: "सक्रिय अभियानों को देखें और देखें कौन निवेश कर रहा है",
    activeCampaigns: "सक्रिय अभियान",
    totalRaised: "कुल जुटाई गई राशि",
    totalBackers: "कुल समर्थक",
    funded: "वित्तपोषित",
    noActiveCampaigns: "अभी कोई सक्रिय अभियान नहीं",
    checkBackSoon: "नई क्राउडफंडिंग अवसरों के लिए जल्द ही फिर आएं!",
    whatsapp: "व्हाट्सएप",
    email: "ईमेल",
    disconnect: "डिसकनेक्ट",
    connected: "कनेक्टेड",
    notProvided: "उपलब्ध नहीं",
    investmentFocus: "निवेश फोकस",
    portfolioDisclaimer: "यह एक सिमुलेटेड प्रोजेक्शन है जो निवेशक कनेक्शनों के बाद संभावित वृद्धि दर्शाता है।",
    
    // Profile page
    myProfile: "मेरी प्रोफ़ाइल",
    yourPersonalDetails: "आपका व्यक्तिगत विवरण",
    fullName: "पूरा नाम",
    email: "ईमेल",
    phoneNumber: "फ़ोन नंबर",
    location: "स्थान",
    businessName: "व्यवसाय का नाम",
    businessType: "व्यवसाय का प्रकार",
    preferredLanguage: "पसंदीदा भाषा",
    aboutMe: "मेरे बारे में",
    saveProfile: "प्रोफ़ाइल सहेजें",
    logout: "लॉगआउट",
    selectLanguage: "भाषा चुनें",
    notSet: "सेट नहीं",
    yourPhoneNumber: "आपका फ़ोन नंबर",
    villageCityState: "गांव / शहर, राज्य",
    yourBusinessName: "आपके व्यवसाय का नाम",
    selectBusinessType: "अपने व्यवसाय का प्रकार चुनें",
    tellUsAboutYourself: "अपने और अपने व्यवसाय के बारे में बताएं...",
    
    // Business types
    handicrafts: "🎨 हस्तशिल्प",
    textiles: "🧵 वस्त्र",
    food: "🍳 खाद्य",
    agriculture: "🌾 कृषि",
    retail: "🛍️ खुदरा",
    services: "💼 सेवाएं",
    other: "📦 अन्य",
    
    // Support
    tollFreeSupport: "टोल-फ्री सहायता",
    available24x7: "24/7 उपलब्ध",
    
    // Investor Dashboard
    welcomeBack: "वापसी पर स्वागत है",
    discoverInvest: "आशाजनक महिला उद्यमियों में खोजें और निवेश करें",
    activePitches: "सक्रिय पिच",
    entrepreneurs: "उद्यमी",
    opportunities: "अवसर",
    growthRate: "वृद्धि दर",
    explorePitches: "पिच एक्सप्लोर करें",
    browseInnovative: "महिला उद्यमियों के नवीन व्यवसाय विचारों को ब्राउज़ करें",
    viewAllPitches: "सभी पिच देखें",
    myPortfolio: "मेरा पोर्टफोलियो",
    trackInvestments: "अपने निवेश और जुड़े उद्यमियों को ट्रैक करें",
    viewPortfolio: "पोर्टफोलियो देखें",
    recentPitches: "हाल की पिच",
    viewAll: "सभी देखें",
    fundingNeeded: "फंडिंग आवश्यक",
    noPitchesAvailable: "कोई हालिया पिच उपलब्ध नहीं",
    coFounderConnector: "सह-संस्थापक कनेक्टर",
    findPerfectPartner: "एक साथ बढ़ने के लिए सही व्यवसाय साझेदार खोजें",
    aiPoweredMatching: "एआई-संचालित मिलान",
    aiAnalyzes: "हमारा एआई आपके कौशल, अनुभव और लक्ष्यों का विश्लेषण करता है",
    searchByName: "नाम, कौशल या स्थान से खोजें...",
    connect: "कनेक्ट करें",
    pitch: "पिच",
    dashboard: "डैशबोर्ड",
    overview: "अवलोकन",
    profileDetails: "प्रोफ़ाइल विवरण",
    connections: "कनेक्शन",
    activity: "गतिविधि",
    saveProfile: "प्रोफ़ाइल सहेजें",
    
    // Home page
    empoweringWomen: "महिलाओं को सशक्त बनाना",
    getStarted: "शुरू करें",
    womenWhoMadeIt: "महिलाएं जिन्होंने सफलता हासिल की",
    voiceFirst: "वॉयस-फर्स्ट इंटरफ़ेस",
    aiCoaching: "एआई कोचिंग",
    investorConnections: "निवेशक कनेक्शन",
    
    // Community
    celebratingAmazingWomen: "हमारी अद्भुत महिला उद्यमियों का जश्न मना रहे हैं",
    topInnovator: "शीर्ष नवप्रवर्तक",
    bestPitch: "सर्वश्रेष्ठ पिच",
    communityHelper: "समुदाय सहायक",
    cofounderConnector: "सह-संस्थापक कनेक्टर",
    recognizingExcellence: "उत्कृष्टता को मान्यता",
    topEntrepreneursThisMonth: "इस महीने के शीर्ष उद्यमी",
    
    // RoleSelect page
    shakti: "शक्ति",
    empoweringRuralWomen: "ग्रामीण महिलाओं को उनके सपनों को संपन्न व्यवसायों में बदलने के लिए सशक्त बनाना",
    selectYourRole: "अपनी भूमिका चुनें",
    imAnEntrepreneur: "मैं एक उद्यमी हूं",
    entrepreneurDescription: "मेरे पास एक व्यवसाय विचार है और मैं इसे दुनिया के साथ साझा करना चाहता हूं, निवेशक खोजना चाहता हूं, और बढ़ना चाहता हूं",
    imAnInvestor: "मैं एक निवेशक हूं",
    investorDescription: "मैं आशाजनक व्यवसाय विचारों की खोज करना चाहता हूं और ग्रामीण महिला उद्यमियों का समर्थन करना चाहता हूं",
    watchDemo: "डेमो देखें",
    needHelp: "मदद चाहिए?",
    callTollFree: "टोल-फ्री कॉल करें",
    selectLanguageToStart: "शुरू करने के लिए अपनी भाषा चुनें",
  },
  ta: {
    // Navigation
    home: "முகப்பு",
    myIdea: "எனது யோசனை",
    findInvestors: "முதலீட்டாளர்களைக் கண்டறியவும்",
    learningHub: "கற்றல் மையம்",
    cofounderConnect: "இணை நிறுவனர் இணைப்பு",
    community: "சமூகம்",
    meetingNotes: "கூட்ட குறிப்புகள்",
    appointments: "சந்திப்புகள்",
    profile: "சுயவிவரம்",
    
    // Profile page
    myProfile: "எனது சுயவிவரம்",
    yourPersonalDetails: "உங்கள் தனிப்பட்ட விவரங்கள்",
    fullName: "முழு பெயர்",
    email: "மின்னஞ்சல்",
    phoneNumber: "தொலைபேசி எண்",
    location: "இடம்",
    businessName: "வணிக பெயர்",
    businessType: "வணிக வகை",
    preferredLanguage: "விருப்பமான மொழி",
    aboutMe: "என்னைப் பற்றி",
    saveProfile: "சுயவிவரத்தைச் சேமி",
    logout: "வெளியேறு",
    selectLanguage: "மொழியைத் தேர்ந்தெடுக்கவும்",
    notSet: "அமைக்கப்படவில்லை",
    yourPhoneNumber: "உங்கள் தொலைபேசி எண்",
    villageCityState: "கிராமம் / நகரம், மாநிலம்",
    yourBusinessName: "உங்கள் வணிக பெயர்",
    selectBusinessType: "உங்கள் வணிக வகையைத் தேர்ந்தெடுக்கவும்",
    tellUsAboutYourself: "உங்களையும் உங்கள் வணிகத்தையும் பற்றி எங்களிடம் சொல்லுங்கள்...",
    
    // Business types
    handicrafts: "🎨 கைவினைப்பொருட்கள்",
    textiles: "🧵 துணிகள்",
    food: "🍳 உணவு",
    agriculture: "🌾 விவசாயம்",
    retail: "🛍️ சில்லறை",
    services: "💼 சேவைகள்",
    other: "📦 மற்றவை",
    
    // Support
    tollFreeSupport: "இலவச ஆதரவு",
    available24x7: "24/7 கிடைக்கும்",
    
    // Community
    celebratingAmazingWomen: "எங்கள் அற்புதமான பெண் தொழில்முனைவோரைக் கொண்டாடுகிறோம்",
    topInnovator: "சிறந்த புதுமையாளர்",
    bestPitch: "சிறந்த பிட்ச்",
    communityHelper: "சமூக உதவியாளர்",
    cofounderConnector: "இணை நிறுவனர் இணைப்பாளர்",
    recognizingExcellence: "சிறப்பை அங்கீகரித்தல்",
    topEntrepreneursThisMonth: "இந்த மாதத்தின் சிறந்த தொழில்முனைவோர்",
    
    // RoleSelect page
    shakti: "சக்தி",
    empoweringRuralWomen: "கிராமப்புற பெண்களை அவர்களின் கனவுகளை செழிப்பான வணிகங்களாக மாற்ற அதிகாரம் அளித்தல்",
    selectYourRole: "உங்கள் பங்கைத் தேர்ந்தெடுக்கவும்",
    imAnEntrepreneur: "நான் ஒரு தொழில்முனைவோன்",
    entrepreneurDescription: "என்னிடம் ஒரு வணிக யோசனை உள்ளது மற்றும் அதை உலகத்துடன் பகிர்ந்து கொள்ள விரும்புகிறேன், முதலீட்டாளர்களைக் கண்டறிய விரும்புகிறேன், மற்றும் வளர விரும்புகிறேன்",
    imAnInvestor: "நான் ஒரு முதலீட்டாளர்",
    investorDescription: "நான் நம்பிக்கைக்குரிய வணிக யோசனைகளைக் கண்டறிய விரும்புகிறேன் மற்றும் கிராமப்புற பெண் தொழில்முனைவோருக்கு ஆதரவளிக்க விரும்புகிறேன்",
  },
  te: {
    // Navigation
    home: "హోమ్",
    myIdea: "నా ఆలోచన",
    findInvestors: "పెట్టుబడిదారులను కనుగొనండి",
    learningHub: "అభ్యాస కేంద్రం",
    cofounderConnect: "సహ వ్యవస్థాపక కనెక్ట్",
    community: "సంఘం",
    meetingNotes: "సమావేశ గమనికలు",
    appointments: "అపాయింట్‌మెంట్లు",
    profile: "ప్రొఫైల్",
    
    // Profile page
    myProfile: "నా ప్రొఫైల్",
    yourPersonalDetails: "మీ వ్యక్తిగత వివరాలు",
    fullName: "పూర్తి పేరు",
    email: "ఇమెయిల్",
    phoneNumber: "ఫోన్ నంబర్",
    location: "స్థానం",
    businessName: "వ్యాపార పేరు",
    businessType: "వ్యాపార రకం",
    preferredLanguage: "ఇష్టమైన భాష",
    aboutMe: "నా గురించి",
    saveProfile: "ప్రొఫైల్ సేవ్ చేయండి",
    logout: "లాగ్అవుట్",
    selectLanguage: "భాషను ఎంచుకోండి",
    notSet: "సెట్ చేయలేదు",
    yourPhoneNumber: "మీ ఫోన్ నంబర్",
    villageCityState: "గ్రామం / నగరం, రాష్ట్రం",
    yourBusinessName: "మీ వ్యాపార పేరు",
    selectBusinessType: "మీ వ్యాపార రకాన్ని ఎంచుకోండి",
    tellUsAboutYourself: "మీ గురించి మరియు మీ వ్యాపారం గురించి మాకు చెప్పండి...",
    
    // Business types
    handicrafts: "🎨 హస్తకళలు",
    textiles: "🧵 వస్త్రాలు",
    food: "🍳 ఆహారం",
    agriculture: "🌾 వ్యవసాయం",
    retail: "🛍️ రిటైల్",
    services: "💼 సేవలు",
    other: "📦 ఇతర",
    
    // Support
    tollFreeSupport: "టోల్-ఫ్రీ మద్దతు",
    available24x7: "24/7 అందుబాటులో ఉంది",
    
    // Community
    celebratingAmazingWomen: "మా అద్భుతమైన మహిళా వ్యవసాయదారులను జరుపుకుంటున్నాము",
    topInnovator: "అగ్ర ఆవిష్కర్త",
    bestPitch: "ఉత్తమ పిచ్",
    communityHelper: "సంఘ సహాయకుడు",
    cofounderConnector: "సహ వ్యవస్థాపక కనెక్టర్",
    recognizingExcellence: "శ్రేష్ఠతను గుర్తించడం",
    topEntrepreneursThisMonth: "ఈ నెలలో అగ్ర వ్యవసాయదారులు",
    
    // RoleSelect page
    shakti: "శక్తి",
    empoweringRuralWomen: "గ్రామీణ మహిళలను వారి కలలను అభివృద్ధి చెందుతున్న వ్యాపారాలుగా మార్చడానికి శక్తివంతం చేయడం",
    selectYourRole: "మీ పాత్రను ఎంచుకోండి",
    imAnEntrepreneur: "నేను వ్యవసాయదారుడిని",
    entrepreneurDescription: "నా వద్ద వ్యాపార ఆలోచన ఉంది మరియు దానిని ప్రపంచంతో పంచుకోవాలనుకుంటున్నాను, పెట్టుబడిదారులను కనుగొనాలి మరియు పెరగాలి",
    imAnInvestor: "నేను పెట్టుబడిదారుడిని",
    investorDescription: "నేను ఆశాజనకమైన వ్యాపార ఆలోచనలను కనుగొనాలనుకుంటున్నాను మరియు గ్రామీణ మహిళా వ్యవసాయదారులకు మద్దతు ఇవ్వాలనుకుంటున్నాను",
  },
  bn: {
    // Navigation
    home: "হোম",
    myIdea: "আমার ধারণা",
    findInvestors: "বিনিয়োগকারী খুঁজুন",
    learningHub: "শেখার কেন্দ্র",
    cofounderConnect: "সহ-প্রতিষ্ঠাতা সংযোগ",
    community: "সম্প্রদায়",
    meetingNotes: "মিটিং নোট",
    appointments: "অ্যাপয়েন্টমেন্ট",
    profile: "প্রোফাইল",
    
    // Profile page
    myProfile: "আমার প্রোফাইল",
    yourPersonalDetails: "আপনার ব্যক্তিগত বিবরণ",
    fullName: "সম্পূর্ণ নাম",
    email: "ইমেল",
    phoneNumber: "ফোন নম্বর",
    location: "অবস্থান",
    businessName: "ব্যবসার নাম",
    businessType: "ব্যবসার ধরন",
    preferredLanguage: "পছন্দের ভাষা",
    aboutMe: "আমার সম্পর্কে",
    saveProfile: "প্রোফাইল সংরক্ষণ করুন",
    logout: "লগআউট",
    selectLanguage: "ভাষা নির্বাচন করুন",
    notSet: "সেট করা নেই",
    yourPhoneNumber: "আপনার ফোন নম্বর",
    villageCityState: "গ্রাম / শহর, রাজ্য",
    yourBusinessName: "আপনার ব্যবসার নাম",
    selectBusinessType: "আপনার ব্যবসার ধরন নির্বাচন করুন",
    tellUsAboutYourself: "আপনার এবং আপনার ব্যবসা সম্পর্কে আমাদের বলুন...",
    
    // Business types
    handicrafts: "🎨 হস্তশিল্প",
    textiles: "🧵 বস্ত্র",
    food: "🍳 খাদ্য",
    agriculture: "🌾 কৃষি",
    retail: "🛍️ খুচরা",
    services: "💼 সেবা",
    other: "📦 অন্যান্য",
    
    // Support
    tollFreeSupport: "টোল-ফ্রি সহায়তা",
    available24x7: "24/7 উপলব্ধ",
    
    // Community
    celebratingAmazingWomen: "আমাদের অসাধারণ মহিলা উদ্যোক্তাদের উদযাপন করছি",
    topInnovator: "শীর্ষ উদ্ভাবক",
    bestPitch: "সেরা পিচ",
    communityHelper: "সম্প্রদায় সহায়ক",
    cofounderConnector: "সহ-প্রতিষ্ঠাতা সংযোগকারী",
    recognizingExcellence: "উৎকর্ষতা স্বীকৃতি",
    topEntrepreneursThisMonth: "এই মাসের শীর্ষ উদ্যোক্তা",
    
    // RoleSelect page
    shakti: "শক্তি",
    empoweringRuralWomen: "গ্রামীণ মহিলাদের তাদের স্বপ্নগুলিকে সমৃদ্ধ ব্যবসায় পরিণত করার ক্ষমতায়ন",
    selectYourRole: "আপনার ভূমিকা নির্বাচন করুন",
    imAnEntrepreneur: "আমি একজন উদ্যোক্তা",
    entrepreneurDescription: "আমার একটি ব্যবসায়িক ধারণা আছে এবং আমি এটি বিশ্বের সাথে ভাগ করতে চাই, বিনিয়োগকারী খুঁজতে চাই এবং বৃদ্ধি করতে চাই",
    imAnInvestor: "আমি একজন বিনিয়োগকারী",
    investorDescription: "আমি প্রতিশ্রুতিশীল ব্যবসায়িক ধারণা আবিষ্কার করতে চাই এবং গ্রামীণ মহিলা উদ্যোক্তাদের সমর্থন করতে চাই",
  },
  mr: {
    // Navigation
    home: "होम",
    myIdea: "माझी कल्पना",
    findInvestors: "गुंतवणूकदार शोधा",
    learningHub: "शिकण्याचे केंद्र",
    cofounderConnect: "सह-संस्थापक कनेक्ट",
    community: "समुदाय",
    meetingNotes: "मीटिंग नोट्स",
    appointments: "भेटी",
    profile: "प्रोफाइल",
    
    // Profile page
    myProfile: "माझे प्रोफाइल",
    yourPersonalDetails: "तुमचे वैयक्तिक तपशील",
    fullName: "पूर्ण नाव",
    email: "ईमेल",
    phoneNumber: "फोन नंबर",
    location: "स्थान",
    businessName: "व्यवसायाचे नाव",
    businessType: "व्यवसायाचा प्रकार",
    preferredLanguage: "पसंतीची भाषा",
    aboutMe: "माझ्याबद्दल",
    saveProfile: "प्रोफाइल जतन करा",
    logout: "लॉगआउट",
    selectLanguage: "भाषा निवडा",
    notSet: "सेट केलेले नाही",
    yourPhoneNumber: "तुमचा फोन नंबर",
    villageCityState: "गाव / शहर, राज्य",
    yourBusinessName: "तुमच्या व्यवसायाचे नाव",
    selectBusinessType: "तुमचा व्यवसाय प्रकार निवडा",
    tellUsAboutYourself: "तुमच्या आणि तुमच्या व्यवसायाबद्दल आम्हाला सांगा...",
    
    // Business types
    handicrafts: "🎨 हस्तकला",
    textiles: "🧵 वस्त्र",
    food: "🍳 अन्न",
    agriculture: "🌾 शेती",
    retail: "🛍️ किरकोळ",
    services: "💼 सेवा",
    other: "📦 इतर",
    
    // Support
    tollFreeSupport: "टोल-फ्री समर्थन",
    available24x7: "24/7 उपलब्ध",
    
    // Community
    celebratingAmazingWomen: "आमच्या अद्भुत महिला उद्योजकांचा उत्सव साजरा करत आहोत",
    topInnovator: "शीर्ष नवोन्मेषक",
    bestPitch: "सर्वोत्तम पिच",
    communityHelper: "समुदाय मदतनीस",
    cofounderConnector: "सह-संस्थापक कनेक्टर",
    recognizingExcellence: "उत्कृष्टता ओळखणे",
    topEntrepreneursThisMonth: "या महिन्याचे शीर्ष उद्योजक",
    
    // RoleSelect page
    shakti: "शक्ती",
    empoweringRuralWomen: "ग्रामीण महिलांना त्यांची स्वप्ने भरभराटीच्या व्यवसायांमध्ये बदलण्यासाठी सशक्त करणे",
    selectYourRole: "तुमची भूमिका निवडा",
    imAnEntrepreneur: "मी एक उद्योजक आहे",
    entrepreneurDescription: "माझ्याकडे एक व्यवसाय कल्पना आहे आणि मला ती जगासोबत शेअर करायची आहे, गुंतवणूकदार शोधायचे आहेत आणि वाढायचे आहे",
    imAnInvestor: "मी एक गुंतवणूकदार आहे",
    investorDescription: "मला आशादायक व्यवसाय कल्पना शोधायच्या आहेत आणि ग्रामीण महिला उद्योजकांना समर्थन द्यायचे आहे",
  },
  gu: {
    // Navigation
    home: "હોમ",
    myIdea: "મારો વિચાર",
    findInvestors: "રોકાણકારો શોધો",
    learningHub: "શીખવાનું કેન્દ્ર",
    cofounderConnect: "સહ-સ્થાપક કનેક્ટ",
    community: "સમુદાય",
    meetingNotes: "મીટિંગ નોંધ",
    appointments: "મુલાકાતો",
    profile: "પ્રોફાઇલ",
    
    // Profile page
    myProfile: "મારી પ્રોફાઇલ",
    yourPersonalDetails: "તમારી વ્યક્તિગત વિગતો",
    fullName: "પૂરું નામ",
    email: "ઇમેઇલ",
    phoneNumber: "ફોન નંબર",
    location: "સ્થાન",
    businessName: "વ્યવસાયનું નામ",
    businessType: "વ્યવસાયનો પ્રકાર",
    preferredLanguage: "પસંદગીની ભાષા",
    aboutMe: "મારા વિશે",
    saveProfile: "પ્રોફાઇલ સાચવો",
    logout: "લૉગઆઉટ",
    selectLanguage: "ભાષા પસંદ કરો",
    notSet: "સેટ નથી",
    yourPhoneNumber: "તમારો ફોન નંબર",
    villageCityState: "ગામ / શહેર, રાજ્ય",
    yourBusinessName: "તમારા વ્યવસાયનું નામ",
    selectBusinessType: "તમારો વ્યવસાય પ્રકાર પસંદ કરો",
    tellUsAboutYourself: "તમારા અને તમારા વ્યવસાય વિશે અમને કહો...",
    
    // Business types
    handicrafts: "🎨 હસ્તકલા",
    textiles: "🧵 કાપડ",
    food: "🍳 ખોરાક",
    agriculture: "🌾 કૃષિ",
    retail: "🛍️ રિટેલ",
    services: "💼 સેવાઓ",
    other: "📦 અન્ય",
    
    // Support
    tollFreeSupport: "ટોલ-ફ્રી સપોર્ટ",
    available24x7: "24/7 ઉપલબ્ધ",
    
    // Community
    celebratingAmazingWomen: "અમારા અદ્ભુત મહિલા ઉદ્યોગસાહસિકોની ઉજવણી કરી રહ્યા છીએ",
    topInnovator: "ટોચના નવીનતાકાર",
    bestPitch: "શ્રેષ્ઠ પિચ",
    communityHelper: "સમુદાય સહાયક",
    cofounderConnector: "સહ-સ્થાપક કનેક્ટર",
    recognizingExcellence: "ઉત્કૃષ્ટતાને માન્યતા",
    topEntrepreneursThisMonth: "આ મહિનાના ટોચના ઉદ્યોગસાહસિકો",
    
    // RoleSelect page
    shakti: "શક્તિ",
    empoweringRuralWomen: "ગ્રામીણ મહિલાઓને તેમના સપનાને સમૃદ્ધ વ્યવસાયોમાં ફેરવવા માટે સશક્તિકરણ",
    selectYourRole: "તમારી ભૂમિકા પસંદ કરો",
    imAnEntrepreneur: "હું એક ઉદ્યોગસાહસિક છું",
    entrepreneurDescription: "મારી પાસે એક વ્યવસાય વિચાર છે અને હું તેને વિશ્વ સાથે શેર કરવા માંગુ છું, રોકાણકારો શોધવા માંગુ છું અને વૃદ્ધિ કરવા માંગુ છું",
    imAnInvestor: "હું એક રોકાણકાર છું",
    investorDescription: "હું આશાસ્પદ વ્યવસાય વિચારો શોધવા માંગુ છું અને ગ્રામીણ મહિલા ઉદ્યોગસાહસિકોને સમર્થન આપવા માંગુ છું",
  },
  kn: {
    // Navigation
    home: "ಹೋಮ್",
    myIdea: "ನನ್ನ ಕಲ್ಪನೆ",
    findInvestors: "ಹೂಡಿಕೆದಾರರನ್ನು ಹುಡುಕಿ",
    learningHub: "ಕಲಿಕಾ ಕೇಂದ್ರ",
    cofounderConnect: "ಸಹ-ಸಂಸ್ಥಾಪಕ ಸಂಪರ್ಕ",
    community: "ಸಮುದಾಯ",
    meetingNotes: "ಸಭಾ ಟಿಪ್ಪಣಿಗಳು",
    appointments: "ಅಪಾಯಿಂಟ್‌ಮೆಂಟ್‌ಗಳು",
    profile: "ಪ್ರೊಫೈಲ್",
    
    // Profile page
    myProfile: "ನನ್ನ ಪ್ರೊಫೈಲ್",
    yourPersonalDetails: "ನಿಮ್ಮ ವೈಯಕ್ತಿಕ ವಿವರಗಳು",
    fullName: "ಪೂರ್ಣ ಹೆಸರು",
    email: "ಇಮೇಲ್",
    phoneNumber: "ಫೋನ್ ಸಂಖ್ಯೆ",
    location: "ಸ್ಥಳ",
    businessName: "ವ್ಯಾಪಾರದ ಹೆಸರು",
    businessType: "ವ್ಯಾಪಾರದ ಪ್ರಕಾರ",
    preferredLanguage: "ಆದ್ಯತೆಯ ಭಾಷೆ",
    aboutMe: "ನನ್ನ ಬಗ್ಗೆ",
    saveProfile: "ಪ್ರೊಫೈಲ್ ಉಳಿಸಿ",
    logout: "ಲಾಗ್ಔಟ್",
    selectLanguage: "ಭಾಷೆಯನ್ನು ಆಯ್ಕೆಮಾಡಿ",
    notSet: "ಹೊಂದಿಸಿಲ್ಲ",
    yourPhoneNumber: "ನಿಮ್ಮ ಫೋನ್ ಸಂಖ್ಯೆ",
    villageCityState: "ಗ್ರಾಮ / ನಗರ, ರಾಜ್ಯ",
    yourBusinessName: "ನಿಮ್ಮ ವ್ಯಾಪಾರದ ಹೆಸರು",
    selectBusinessType: "ನಿಮ್ಮ ವ್ಯಾಪಾರ ಪ್ರಕಾರವನ್ನು ಆಯ್ಕೆಮಾಡಿ",
    tellUsAboutYourself: "ನಿಮ್ಮ ಮತ್ತು ನಿಮ್ಮ ವ್ಯಾಪಾರದ ಬಗ್ಗೆ ನಮಗೆ ತಿಳಿಸಿ...",
    
    // Business types
    handicrafts: "🎨 ಕರಕುಶಲ",
    textiles: "🧵 ಜವಳಿ",
    food: "🍳 ಆಹಾರ",
    agriculture: "🌾 ಕೃಷಿ",
    retail: "🛍️ ಚಿಲ್ಲರೆ",
    services: "💼 ಸೇವೆಗಳು",
    other: "📦 ಇತರೆ",
    
    // Support
    tollFreeSupport: "ಟೋಲ್-ಫ್ರೀ ಬೆಂಬಲ",
    available24x7: "24/7 ಲಭ್ಯವಿದೆ",
    
    // Community
    celebratingAmazingWomen: "ನಮ್ಮ ಅದ್ಭುತ ಮಹಿಳಾ ಉದ್ಯಮಿಗಳನ್ನು ಆಚರಿಸುತ್ತಿದ್ದೇವೆ",
    topInnovator: "ಉನ್ನತ ನವೋನ್ಮೇಷಕ",
    bestPitch: "ಅತ್ಯುತ್ತಮ ಪಿಚ್",
    communityHelper: "ಸಮುದಾಯ ಸಹಾಯಕ",
    cofounderConnector: "ಸಹ-ಸಂಸ್ಥಾಪಕ ಸಂಪರ್ಕಕಾರ",
    recognizingExcellence: "ಉತ್ಕೃಷ್ಟತೆಯನ್ನು ಗುರುತಿಸುವುದು",
    topEntrepreneursThisMonth: "ಈ ತಿಂಗಳ ಉನ್ನತ ಉದ್ಯಮಿಗಳು",
    
    // RoleSelect page
    shakti: "ಶಕ್ತಿ",
    empoweringRuralWomen: "ಗ್ರಾಮೀಣ ಮಹಿಳೆಯರನ್ನು ಅವರ ಕನಸುಗಳನ್ನು ಅಭಿವೃದ್ಧಿ ಹೊಂದುತ್ತಿರುವ ವ್ಯಾಪಾರಗಳಾಗಿ ಪರಿವರ್ತಿಸಲು ಶಕ್ತಿಯುತಗೊಳಿಸುವುದು",
    selectYourRole: "ನಿಮ್ಮ ಪಾತ್ರವನ್ನು ಆಯ್ಕೆಮಾಡಿ",
    imAnEntrepreneur: "ನಾನು ಉದ್ಯಮಿ",
    entrepreneurDescription: "ನನ್ನ ಬಳಿ ವ್ಯಾಪಾರ ಕಲ್ಪನೆ ಇದೆ ಮತ್ತು ಅದನ್ನು ಪ್ರಪಂಚದೊಂದಿಗೆ ಹಂಚಿಕೊಳ್ಳಲು ಬಯಸುತ್ತೇನೆ, ಹೂಡಿಕೆದಾರರನ್ನು ಹುಡುಕಲು ಮತ್ತು ಬೆಳೆಯಲು ಬಯಸುತ್ತೇನೆ",
    imAnInvestor: "ನಾನು ಹೂಡಿಕೆದಾರ",
    investorDescription: "ನಾನು ಭರವಸೆಯ ವ್ಯಾಪಾರ ಕಲ್ಪನೆಗಳನ್ನು ಕಂಡುಹಿಡಿಯಲು ಮತ್ತು ಗ್ರಾಮೀಣ ಮಹಿಳಾ ಉದ್ಯಮಿಗಳನ್ನು ಬೆಂಬಲಿಸಲು ಬಯಸುತ್ತೇನೆ",
  },
  ml: {
    // Navigation
    home: "ഹോം",
    myIdea: "എന്റെ ആശയം",
    findInvestors: "നിക്ഷേപകരെ കണ്ടെത്തുക",
    learningHub: "പഠന കേന്ദ്രം",
    cofounderConnect: "സഹ-സ്ഥാപക കണക്ട്",
    community: "കമ്മ്യൂണിറ്റി",
    meetingNotes: "മീറ്റിംഗ് കുറിപ്പുകൾ",
    appointments: "അപ്പോയിന്റ്‌മെന്റുകൾ",
    profile: "പ്രൊഫൈൽ",
    
    // Profile page
    myProfile: "എന്റെ പ്രൊഫൈൽ",
    yourPersonalDetails: "നിങ്ങളുടെ വ്യക്തിഗത വിവരങ്ങൾ",
    fullName: "പൂർണ്ണ നാമം",
    email: "ഇമെയിൽ",
    phoneNumber: "ഫോൺ നമ്പർ",
    location: "സ്ഥലം",
    businessName: "ബിസിനസ് പേര്",
    businessType: "ബിസിനസ് തരം",
    preferredLanguage: "ഇഷ്ട ഭാഷ",
    aboutMe: "എന്നെക്കുറിച്ച്",
    saveProfile: "പ്രൊഫൈൽ സംരക്ഷിക്കുക",
    logout: "ലോഗൗട്ട്",
    selectLanguage: "ഭാഷ തിരഞ്ഞെടുക്കുക",
    notSet: "സജ്ജമാക്കിയിട്ടില്ല",
    yourPhoneNumber: "നിങ്ങളുടെ ഫോൺ നമ്പർ",
    villageCityState: "ഗ്രാമം / നഗരം, സംസ്ഥാനം",
    yourBusinessName: "നിങ്ങളുടെ ബിസിനസ് പേര്",
    selectBusinessType: "നിങ്ങളുടെ ബിസിനസ് തരം തിരഞ്ഞെടുക്കുക",
    tellUsAboutYourself: "നിങ്ങളെക്കുറിച്ചും നിങ്ങളുടെ ബിസിനസിനെക്കുറിച്ചും ഞങ്ങളോട് പറയുക...",
    
    // Business types
    handicrafts: "🎨 കരകൗശല വസ്തുക്കൾ",
    textiles: "🧵 തുണിത്തരങ്ങൾ",
    food: "🍳 ഭക്ഷണം",
    agriculture: "🌾 കൃഷി",
    retail: "🛍️ റീട്ടെയിൽ",
    services: "💼 സേവനങ്ങൾ",
    other: "📦 മറ്റുള്ളവ",
    
    // Support
    tollFreeSupport: "ടോൾ-ഫ്രീ സപ്പോർട്ട്",
    available24x7: "24/7 ലഭ്യമാണ്",
    
    // Community
    celebratingAmazingWomen: "ഞങ്ങളുടെ അത്ഭുതകരമായ സ്ത്രീ സംരംഭകരെ ആഘോഷിക്കുന്നു",
    topInnovator: "ടോപ് ഇന്നൊവേറ്റർ",
    bestPitch: "മികച്ച പിച്ച്",
    communityHelper: "കമ്മ്യൂണിറ്റി സഹായി",
    cofounderConnector: "സഹ-സ്ഥാപക കണക്ടർ",
    recognizingExcellence: "മികവ് തിരിച്ചറിയൽ",
    topEntrepreneursThisMonth: "ഈ മാസത്തെ മുൻനിര സംരംഭകർ",
    
    // RoleSelect page
    shakti: "ശക്തി",
    empoweringRuralWomen: "ഗ്രാമീണ സ്ത്രീകളെ അവരുടെ സ്വപ്നങ്ങൾ അഭിവൃദ്ധി പ്രാപിക്കുന്ന ബിസിനസുകളാക്കി മാറ്റാൻ ശാക്തീകരിക്കുന്നു",
    selectYourRole: "നിങ്ങളുടെ റോൾ തിരഞ്ഞെടുക്കുക",
    imAnEntrepreneur: "നാൻ ഒരു സംരംഭകനാണ്",
    entrepreneurDescription: "എനിക്ക് ഒരു ബിസിനസ് ആശയമുണ്ട്, അത് ലോകവുമായി പങ്കിടാൻ ആഗ്രഹിക്കുന്നു, നിക്ഷേപകരെ കണ്ടെത്താനും വളരാനും ആഗ്രഹിക്കുന്നു",
    imAnInvestor: "ഞാൻ ഒരു നിക്ഷേപകനാണ്",
    investorDescription: "പ്രതീക്ഷ നൽകുന്ന ബിസിനസ് ആശയങ്ങൾ കണ്ടെത്താനും ഗ്രാമീണ സ്ത്രീ സംരംഭകരെ പിന്തുണയ്ക്കാനും ഞാൻ ആഗ്രഹിക്കുന്നു",
  },
};

export function LanguageProvider({ children }) {
  const [language, setLanguage] = useState("en");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Load user's preferred language
    base44.auth.me()
      .then((user) => {
        if (user?.app_language) {
          setLanguage(user.app_language);
        }
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, []);

  const changeLanguage = async (lang) => {
    setLanguage(lang);
    try {
      await base44.auth.updateMe({ app_language: lang });
    } catch (error) {
      console.error("Failed to save language preference:", error);
    }
  };

  const t = (key) => {
    return translations[language]?.[key] || translations.en[key] || key;
  };

  if (loading) {
    return null;
  }

  return (
    <LanguageContext.Provider value={{ language, changeLanguage, t }}>
      {children}
    </LanguageContext.Provider>
  );
}