...
function LayoutInner({ children, currentPageName, user }) {
  ...
  return (
    <div className="min-h-screen flex" style={{background: 'linear-gradient(135deg, #F5E6EA 0%, #FCF8F9 100%)'}}>
      <style>{`
        :root {
          --color-primary: #DC586D;
          --color-secondary: #A8515B;
          --color-deep: #A33757;
        }
        ...
        ::-webkit-scrollbar-track {
          background: rgba(245, 230, 234, 0.5);
        }
        ::-webkit-scrollbar-thumb {
          background: rgba(168, 81, 91, 0.5);
          border-radius: 4px;
        }
      ...
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 sidebar-berry border-b border-white/20 flex items-center justify-between px-4 z-50 shadow-md">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl overflow-hidden border border-white/30 shadow-sm">
            <img 
      ...
      {/* Sidebar */}
      <nav
        className={`fixed md:static top-0 left-0 h-full w-64 sidebar-berry border-r border-white/10 z-50 transform transition-transform duration-300 shadow-lg ${
          mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <div className="p-5">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/30 shadow-md">
              <img 
                src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/6988ce510df9dcc25137f48f/f64248ae0_WhatsAppImage2026-02-10at1233471.jpeg" 
                alt="Shakti Logo" 
                className="w-full h-full object-cover"
            ...
            <div>
              ...
              <p className="text-xs text-white/70">Empowering Women</p>
            </div>
          </div>
